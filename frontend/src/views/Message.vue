      ...
      <section class="col-span-12 flex h-[80vh] flex-col rounded-3xl border border-white/70 bg-white/90 shadow-[0_20px_55px_-26px_rgba(15,23,42,0.5)] backdrop-blur-xl md:col-span-8 lg:col-span-6">
        ...
        >
          ...
          <textarea
            ...
            :disabled="sending || (isDirectChat && (blockedMe || blockedByMe))"
          ></textarea>
          ...
          <div class="flex flex-wrap items-center justify-between gap-3">
            <label
              ...
              :class="sending || (isDirectChat && (blockedMe || blockedByMe)) ? 'pointer-events-none opacity-60' : ''"
            >
              <input
                type="file"
                accept="image/*,video/*"
          ...
          <p v-else-if="isDirectChat && blockedByMe" class="text-xs text-red-500">You blocked this user, so messaging is disabled.</p>
        </form>
      </section>
...
const sendMessage = async () => {
  ...
  if (isDirectChat.value && blockedByMe.value) {
    errorMessage.value = 'You blocked this user, so messaging is disabled.'
    return
  }
...
const loadInitialConversation = async (contactId = route.params.userId) => {
  try {
    await loadMe()
    connectMessageSocket()
...
watch(
  ...
  async (userId) => {
    ...
    await loadInitialConversation(userId)
  },
)
